...
utility_nodes = node.engineyard.environment.instances.select { |i| ["util"].include?(i["role"]) }.map { |i| { "ey-utility-#{i['name']}".gsub(/-$/, "") => node.private_ip_for(i) } }.each_with_object({}) { |el, h| el.each { |k, v| h[k].nil? ? h[k] = v : h[k] = (Array.new([h[k]]) << v).compact.flatten } }

# DB Replicas by instance name (ey-db-slave-#{name}[-X]) where X is some number if name is not unique
db_replicas = node.engineyard.environment.instances.select { |i| ["db_slave"].include?(i["role"]) and !(i["name"].nil? or i["name"] == "") }.map { |i| { "ey-db-slave-#{i['name']}".gsub(/-$/, "") => node.private_ip_for(i) } }.each_with_object({}) { |el, h| el.each { |k, v| h[k].nil? ? h[k] = v : h[k] = (Array.new([h[k]]) << v).compact.flatten } }
