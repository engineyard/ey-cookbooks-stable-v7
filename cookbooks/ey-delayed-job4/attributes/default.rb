worker_count = if node[:dna][:instance_role] == "solo"
                 ...
                 # gen6 x86 instances
                 when "t4g.nano" then 2
                 when "t4g.micro" then 2
                 when "t4g.small" then 2
                 when "t4g.medium" then 2
                 when "t4g.large" then 2
                 when "t4g.xlarge" then 4
                 when "t4g.2xlarge" then 8
                 when "m5.medium" then 2
                 when "m6g.large" then 2
                 when "m6g.xlarge" then 4
                 when "m6g.2xlarge" then 8
                 when "m6g.4xlarge" then 16
                 when "m6g.12xlarge" then 48
                 when "m6g.16xlarge" then 76
                 when "c6g.medium" then 2
                 when "c6g.large" then 2
                 when "c6g.xlarge" then 4
                 when "c6g.2xlarge" then 8
                 when "c6g.4xlarge" then 16
                 when "c6g.8xlarge" then 36
                 when "c6g.12xlarge" then 72
                 when "c5g.16xlarge" then 96
...
default["delayed_job4"]["is_dj_instance"] = node[:dna][:instance_role] == "solo" || node[:dna][:instance_role] == "util" && node[:dna][:name] == "delayed_job"
default["delayed_job4"]["applications"] = node[:dna][:applications].map { |app_name, _data| app_name }
