...
module EphemeralHelpers
  ...
  def _list_xvd_ephemeral_devices(start_index: 1)
    ["xvdd", "xvde"].each_with_index.select { |dev, _i| File.exist?("/dev/#{dev}") }.map { |dev, _i| _build_ephemeral_device("/dev/#{dev}", start_index + i) }
  end

  def _list_nvme_ephemeral_devices(start_index: 1)
    return [] unless ::Dir.exist?("/dev/disk/by-id")
    ::Dir.entries("/dev/disk/by-id").select { |dev| dev.match(/^nvme-ephemeral-[a-zA-Z0-9]+$/) }.sort.each_with_index.map { |dev, i| _build_ephemeral_device("/dev/disk/by-id/#{dev}", start_index + i) }
  end
