...
if node.engineyard.instance.primary_database_server?
  ...
  execute "Setup Server SSL CA and root certificate" do
    ...
    not_if { ::File.exist?(::File.join(ssldir, "root.crt")) }
  end
  ...
  execute "Setup #{owner} user SSL key" do
    ...
    only_if { ::File.exist?(::File.join(ssldir, "root.crt")) }
    not_if { ::File.exist?(::File.join(dbroot, "keygen", owner, "#{keyname}.key")) }
  end
  ...
  execute "Setup #{node['dna']['users'].first['username']} user SSL key" do
    ...
    only_if { ::File.exist?(::File.join(ssldir, "root.crt")) }
    not_if { ::File.exist?(File.join(dbroot, "keygen", node["dna"]["users"].first["username"], "#{keyname}.key")) }
  end
  ...
  execute "Setup user SSL key on all instances" do
    ...
    only_if { ::File.exist?(::File.join(dbroot, "keygen", node["dna"]["users"].first["username"], "#{keyname}.key")) }
  end
...
execute "Copy db ssl keys for #{owner} from EBS if available" do
  ...
  only_if { ::File.exist?(File.join(ssldir, "root.crt")) and ::File.exist?(::File.join(dbroot, "keygen", owner, "#{keyname}.key")) }
end
