...
include_recipe "ey-collectd::httpd"
...
include_recipe "ey-collectd::perl"
...
existing_rrd_datadirs = Dir[File.join(rrd_basedir, "*")]
                        ...
                        .sort_by { |fm| fm[1] }
                        .map { |fm| fm[0] }
latest_rrd_datadir = if existing_rrd_datadirs.empty?
                       nil
                     else
                       File.expand_path(existing_rrd_datadirs[-1])
                     end
if !File.exist?(rrd_datadir) && latest_rrd_datadir
...
execute "install-graphs-app" do
  ...
  not_if do
    File.exist?(node["collectd"]["graph"]["version_file"]) and
