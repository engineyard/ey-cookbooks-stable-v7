...
execute "installer" do
  ...
  #action :nothing
  notifies :install, "apt_package[datadog-signing-keys]", :immediate
...
apt_package 'datadog-agent' do
   action :nothing
   notifies :create, "template[datadog.yaml]", :immediate
end
...
template "Logs" do
  ...
  #action :nothing
  notifies :enable, "service[datadog-agent]", :immediate
  notifies :restart, "service[datadog-agent]", :immediate
end
...
template "syslog-ng.conf" do
 ...
 instance = node["dna"]["engineyard"]["environment"]["instances"].find { |instance| instance["id"] == this }
  ...
  notifies :restart, "service[syslog-ng]", :immediate
end
